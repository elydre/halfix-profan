...
static void render_windows(void)
{
    ...
    for (int i = 0; i < num_subwindows; i++) {
        ...
        for (int j = 0; j < scanlines; j++) {
            hcpy(&pix[pos], &sw->pixbuf[sw_pos], line_length * SCREEN_BYTPP);
...
static int create_subwindow(int chrwidth, int chrheight, int x, int y)
{
    ...
    for (int i = 0; i < MAX_SUBWINDOWS; i++) {
        ...
        if (!sw->visible) {
            ...
            sw->textbuf = halloc(chrsize << 1);
            sw->pixbuf = halloc(chrsize * (CHAR_WIDTH * CHAR_HEIGHT) * SCREEN_BYTPP);
...
static void realloc_bar_sizes(int width)
{
    ...
    if (menubar_width != new_menubar_width) {
        if (menubar_textbuffer)
            ...
            menubar_textbuffer = halloc(new_menubar_width << 1);

        // Menubar width
        if (statusbar_textbuffer)
            statusbar_textbuffer = realloc(statusbar_textbuffer, new_menubar_width << 1);
        else
            statusbar_textbuffer = halloc(new_menubar_width << 1);
    }
    ...
    for (int i = 0; i < MAX_MENUBAR_ITEMS; i++) {
        if (!menubar[i].name)
            continue;
        int rpos = menubar[i].cpos << 1, len = strhlen(menubar[i].name);
        // Add spacer before
...
{
    struct menubar_listing* mbl = halloc(sizeof(struct menubar_listing));
    mbl->nent = sz;
    mbl->entries = challoc(sizeof(char*), sz);
    mbl->offsets = challoc(sizeof(int), sz);
    mbl->cbs = challoc(sizeof(menu_click_cb), sz);
...
static void finalize_menubar(struct menubar_listing* mbl)
{
    ...
    sw.cw = width;
    sw.ch = height;
    uint8_t* textbuf = sw.textbuf = challoc(width * height, 2);
